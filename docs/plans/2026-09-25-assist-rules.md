...
```
ASSIST_FP_RATE {"normal_rows":7,"r1_r2_findings":0,"keys":[]}
ASSIST_P95     {"samples":30,"p50":2,"p95":4,"max":4}   // 单位 ms，预算 < 50 ms
```

- **反向用例**：7 条含 `ɒ̃ Ǿ ʔ`、上标数字调号与合法占位符的正常莆仙条目，R1/R2 命中 **0** 条。
- **单条重算 p95 = 4 ms**（30 次采样，走 HTTP 计时，含读页、读项目键盘配置、下线旧批次、
  插入新批次的全部开销），满足 #177 的提交路径预算。

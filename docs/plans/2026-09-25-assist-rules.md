...
- **单条重算不得抹掉列级疑点**：列级疑点也挂在某一条上，但单条重算判定不了整列，
  所以下线范围用 `COLUMN_MESSAGE_KEYS` 排除它们（`assist_writer.js:pageScope`）。
  不排除的话，"给某一条补算"会顺手把挂在那一条上的列级疑点标下线，而只有项目重算会再产出——
  那是静默永久丢失。这个"排除表恰好等于带列级 anchor 的 key"的一致性由纯函数段断言。
...
- 未测：10k 行项目的**规则全量**耗时。那需要真造 10k 条目，本 PR 的测试项目只有 3 条。
  #178 在同一台机器上量到跨行重算 10k 行 = 6.1 s（`measure_identity_scale.py`），
  只能当同量级参考，不能替 #177 报数。游标已经保证"扫不完就整批拒算"，
  所以这个数字将来只会影响耗时，不会影响正确性。
